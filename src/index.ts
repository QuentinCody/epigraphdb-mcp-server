...
import { EpiGraphDbDataDO } from "./do";

export { EpiGraphDbDataDO };

interface EpiGraphDbEnv {
...
export class MyMCP extends StatelessMcpWorker {
    ...
    async init() {
        ...
        const env = this.env as unknown as EpiGraphDbEnv;
...
export default {
    fetch(request: Request, env: Env, ctx: ExecutionContext) {
        ...
        if (url.pathname === "/health") {
            return buildHealthResponse("epigraphdb");
        }
        ...
        if (url.pathname === "/readyz") {
            ...
            return MyMCP.readiness(env, "epigraphdb");
        }
